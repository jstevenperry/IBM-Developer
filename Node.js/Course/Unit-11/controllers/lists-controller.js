...
function renderViewPage(req, res, next) {
    ...
    async.parallel({
        ...
        let whenCreated = dateformat(new Date(results.list.when_created), 'mm-dd-yyyy hh:MM:ss');
        let whenModified = (results.list.whenModified) ? dateformat(new Date(results.list.when_modified), 'mm-dd-yyyy hh:MM:ss') : null;
        // Now render the page
...
function renderUpdatePage(req, res, next) {
    ...
    request('GET', '/rest/lists/'+listId, null, (err, data) => {
        if (err) {
            ...
            let jsonData = JSON.parse(data);
            let whenCreated = dateformat(new Date(jsonData.when_created), 'mm-dd-yyyy hh:MM:ss');
            let whenModified = (jsonData.whenModified) ? dateformat(new Date(jsonData.when_modified), 'mm-dd-yyyy hh:MM:ss') : null;
            // Now render the page
...
const addListItems = [
    (req, res, next) => {
        ...
        if (itemsToAdd.length > 0) {
            async.each(itemsToAdd, function(id, callback) {
                request('POST', '/rest/lists/'+listId+'/items', `{ "itemId": ${id}, "quantity": 1 }`, (err, data) => {
                    if (err) {
                        next(err);
                    } else {
                        callback();
...
const removeItems = [
    (req, res, next) => {
        ...
        if (itemsToDelete.length > 0) {
            async.each(itemsToDelete, function(id, callback) {
